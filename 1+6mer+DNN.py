...
def create_model_multiclass(input_dim, output_dim):
    # create model
    inputdata = Input(shape=input_dim)
    x1 = Dense(256, activation='selu')(inputdata)
    x1 = Dropout(0.1)(x1)
    x2 = Dense(128, activation='selu')(x1)
    x2 = Dropout(0.1)(x2)
    x_out = layers.concatenate([x1, x2])
    x = Dense(64, activation='selu')(x_out)
