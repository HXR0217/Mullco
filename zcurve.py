...
def z_curve_144bit_sequences(sequences):
    ...
    return np.array(z)
id_train_x = read_fasta('training_validation_seqs')
...
z_y= y
df = pd.DataFrame(z_x)
df.to_csv('train_x_z_48bit.csv', index=False, header=False, sep=" ")
print(z_x.shape)
df = pd.DataFrame(z_y)
df.to_csv('train_y_z_48bit.csv', index=False, header=False, sep=" ")

z_x= np.array(z_curve_144bit_sequences(x))
z_y= y
df = pd.DataFrame(z_x)
df.to_csv('train_x_z_144bit.csv', index=False, header=False, sep=" ")
print(z_x.shape)
df = pd.DataFrame(z_y)
df.to_csv('train_y_z_144bit.csv', index=False, header=False, sep=" ")
